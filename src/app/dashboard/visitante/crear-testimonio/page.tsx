...
export default function CrearTestimonioPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        try {
            ...
            // Add user details if available (required by backend even for logged in users apparently)
            const user = useAuthStore.getState().user;
            if (user) {
                if (user.username) formData.append('usuario_anonimo_username', user.username);
                if (user.email) formData.append('usuario_anonimo_email', user.email);
            }
