      ...
      <Card className="max-w-2xl w-full p-8 md:p-12 text-center space-y-6 shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
          <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/50">
            ...
            <p className="text-sm font-medium">{t("statusCards.dev.title")}</p>
            <p className="text-xs text-muted-foreground text-center">{t("statusCards.dev.desc")}</p>
          </div>

          <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/50">
            <Clock className="h-8 w-8 text-blue-500" />
            <p className="text-sm font-medium">{t("statusCards.soon.title")}</p>
            <p className="text-xs text-muted-foreground text-center">{t("statusCards.soon.desc")}</p>
          </div>

          <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/50">
            <Construction className="h-8 w-8 text-yellow-500" />
            <p className="text-sm font-medium">{t("statusCards.improve.title")}</p>
            <p className="text-xs text-muted-foreground text-center">{t("statusCards.improve.desc")}</p>
          </div>
        </div>
